...
# Prefer local wheels in ./build-tools/pip-cache (filled by build-tools/prefetch.sh),
# fall back to PyTorch CDN. COPY of an empty dir is a valid no-op; if no wheel
# exists locally, pip will fall through to the index for each package.
COPY build-tools/pip-cache/ /pip-cache/
...
# Try local cache first (./build-tools/ffmpeg-cache/ffmpeg-<arch>.tar.xz,
# generated by ./build-tools/prefetch.sh), fall back to network.
# COPY of an empty directory is valid; if no cache file exists, the
# if-then branch falls through to the curl path below.
COPY build-tools/ffmpeg-cache/ /ffmpeg-cache/
...
RUN set -eux \
    ...
    && local_cache="/ffmpeg-cache/ffmpeg-${arch}.tar.xz" \
    && if [ -s "$local_cache" ]; then \
            echo "[cache] using $local_cache (skipped network)"; \
            cp "$local_cache" "$tmpdir/ffmpeg.tar.xz"; \
        else \
            echo "[net  ] downloading $ffmpeg_url"; \
            curl -fsSL \
                --http1.1 \
                --retry 5 \
                --retry-delay 3 \
                ...
                --connect-timeout 30 \
                --max-time 900 \
                "$ffmpeg_url" -o "$tmpdir/ffmpeg.tar.xz"; \
        fi \
